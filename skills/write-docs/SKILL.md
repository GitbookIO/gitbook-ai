...
### Querying GitBook Documentation

If you need information not covered by the above or the reference files, query GitBook's documentation directly:

```
GET https://gitbook.com/docs/getting-started/readme.md?ask=<question>&goal=<endgoal>
```

`ask` is the specific question; `goal` (optional) describes the broader task — GitBook uses it to tailor the answer.
